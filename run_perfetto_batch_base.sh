...
# 增加功能：支持参数控制是否开启 Perfetto (默认关闭)
# 用法：
#   ./run_perfetto_batch.sh          # 仅运行 benchmark，不抓 trace
#   ./run_perfetto_batch.sh --trace  # 运行 benchmark 并抓取 trace
# ============================================================
...
# 只有在开启 trace 时才需要创建结果目录（或者你可以保留，用于存放 log）
...
for case in "${TEST_CASES[@]}"; do
    ...
    # ---------------------------------------------------------
    # 步骤 1: 启动 Perfetto (后台模式) - 仅在开启 Trace 时执行
    # ---------------------------------------------------------
    if [ "$ENABLE_TRACE" = true ]; then
    ...
    # ---------------------------------------------------------
    # 步骤 3, 4, 5: 停止/等待/拉取 - 仅在开启 Trace 时执行
    # ---------------------------------------------------------
    if [ "$ENABLE_TRACE" = true ]; then
